...
2. 正在请求的URL。
3. 请求头，包含了一些客户端环境的信息，和身份验证的信息。
4. 请求体，也就是请求正文，请求正文中可以包含客户提交的查询字符串信息等，表单信息。

##### 一个HTTP响应有三部分组成：
1. 一个数字和文字组成的状态码，用来显示请求的是成功还是失败。
2. 响应头，响应头也和请求头一样包含许多有用的信息，例如服务器类型、日期类型、日期时间、内容类型和长度等
3. 响应体，也就是响应正文。

### JSON
可使用JSONLint进行json格式的数据的校验。